...
bool isPower,firstVriable=true;



int check_type(char item)
{
    int i=-1;
    while(++i < 10){
...
int separte_variable_and_number(string str)
{
    ...
    char store[SIZE];
        ...
        while(initial<=(int)str.size()){
            ...
            if(check_type(str[initial]) == SPACE){              //If found space it will skip and check next
                //continue;
            }
            else if(check_type(str[initial]) == NUMBER){
                if(isPower){
            ...
            else if(check_type(str[initial]) == FRACTIONAL_POINT){
                decimal_point = true;
            }
            else if(check_type(str[initial]) == CHARACTER){
                store[store_index++] = str[initial];
            }
            else if(check_type(str[initial]) == POWER){            //there found a power
                isPower = true;
